  ...
  for(clim.scn in c("rcp45", "rcp85")){
    for(clim.mdl in c("KNMI-RACMO22E_ICHEC-EC-EARTH",
      ...
      for(decade in seq(10,90,10)){ 
        ...
        load(paste0(work.path, "/Medfire/inputlyrs/rdata/climate_", clim.scn, "_", clim.mdl, "_", decade, ".rdata"))
        save(clim, file=paste0(work.path, "/Medfire/inputlyrs/rdata/climate_", clim.scn, "_", clim.mdl, "_", decade, "_CAT.rdata"))
        
        ## Filter data for the new study area
        clim <- left_join(clim, new.mask, by="cell.id") %>% filter(x==1) %>% select(-x)
        
        ## Save it
        save(clim, file=paste0(work.path, "/Medfire/inputlyrs/rdata/climate_", clim.scn, "_", clim.mdl, "_", decade, ".rdata"))
...
read.climatic.vars2 <- function(work.path, model){
  ...
  ## Mask of the study area
  load("Medfire/inputlyrs/rdata/mask_bcn.rdata")
  MASK = MASK_BCN
  ...
  for(clim.scn in c("rcp45", "rcp85")){
    for(decade in seq(00,90,10)){
    	...
    	for(var in c("PRCPTOT", "TNMM")){        
	        ...
	        if(var == "TNMM"){ #PRCPTOT_rcp45_KNMI-RACMO22E_ICHEC-EC-EARTH_proj00_1000m
	          if(decade == 0){
	            TEMP <- raster(paste0(work.path, "/Medfire/inputlyrs/asc/clima_to_oriol/", var,"_",  clim.scn, "_", model,"_proj00_1000m.asc"))
	          }
	          else{
	            TEMP <- raster(paste0(work.path, "/Medfire/inputlyrs/asc/clima_to_oriol/", var,"_",  clim.scn, "_", model,"_proj", decade, "_1000m.asc"))
	            }
	          ...
	          if(decade == 0){
	            PRECIP <- raster(paste0(work.path, "/Medfire/inputlyrs/asc/clima_to_oriol/", var,"_",  clim.scn, "_", model,"_proj00_1000m.asc"))
	          }
	          else{
	            PRECIP <- raster(paste0(work.path, "/Medfire/inputlyrs/asc/clima_to_oriol/", var,"_",  clim.scn, "_", model,"_proj", decade, "_1000m.asc"))
	            }
    ...
    save(clim, file=paste0("Medfire/inputlyrs/rdata/climate_", clim.scn, "_", model, "_", decade, ".rdata"))
    } 
  } 
...
read.climatic.vars <- function(work.path){
  ...
  for(clim.scn in c("rcp45", "rcp85")){
    for(decade in seq(10,90,10)){
      ...
      ## Read annual minimum temp, annual precip and annual solar radiation
      ## Change extend to match the default
      TEMP <- raster(paste0(work.path, "/Medfire/inputlyrs/asc/", clim.scn, "/", decade, "/mnan.asc"))
      TEMP <- extend(TEMP, extCat)
      PRECIP <- raster(paste0(work.path, "/Medfire/inputlyrs/asc/", clim.scn, "/", decade, "/plan.asc"))
      PRECIP <- extend(PRECIP, extCat)
      ...
      save(clim, file=paste0("Medfire/inputlyrs/rdata/climate_", clim.scn, "_", decade, ".rdata"))
