...
update.clim <- function(MASK, land, orography, decade, clim.scn, psdm){
  ...
  clim <- dplyr::select(clim, cell.id, spp, temp, precip, sdm, aspect, slope) %>% 
          ...
          dplyr::select(cell.id, spp, temp, precip, sdm, sqi)
  
  ## SQI for shrubs
  sqi.shrub <- filter(clim, spp==14) %>% dplyr::select(spp, temp, precip) %>% left_join(site.quality.shrub, by="spp") %>%
